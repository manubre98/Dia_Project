...
def allTogether(acc_sigma=10, cost_sigma=0.05, T=365, noe=100, conf_coef=50, rw_coef=1, opt=19600.23):
    exp_params = {'n_arms': 10, 'noe': noe, 'T': T, 'return_time': 30, 'first2': [False, False, False, False]}
    bidding_params = {'min_bid': 0.2, 'max_bid': 2, 'acc_sigma': acc_sigma, 'cost_sigma': cost_sigma}
    ...
    for e in tqdm(range(exp_params['noe']), desc='Number of experiments'):
        ...
        # Create Learner
        pricing_learner = TS_Learner7(exp_params['n_arms'], rw_coef=rw_coef, conf_coef=conf_coef)
        ...
        # SECONDO CICLO - SECONDA DOSE ETEROLOGA
        # print('SECOND CYCLE')
        rewards_c2 = []
        for d in range(starting_point, exp_params['T']):
            starting_point = d + 1
            ...
            bidding_pulled_arm = bidding_learner.pull_arm_context(learners_list=LEARNERS_LIST,
                                                                  ...
                                                                  margins=[margin(prices[p]) for p in
                                                                           pricing_pulled_arms],
            ...
            for index, pricing_learner in enumerate(LEARNERS_LIST):
                ...
                # ts_rewards.append(ts_daily['reward'])
                update_splitting_table(pricing_learner=pricing_learner, returns=returns, clicks=clicks,
                                       purchases=purchases,
        ...
        for d in range(starting_point, exp_params['T']):
            ...
            for index, pricing_learner in enumerate(LEARNERS_LIST):
                ...
                if check_convergence(return_arms[index])[0]:
                    ...
                    # print('\nIndex:', index)
                    # print('Convergence price', check_convergence(return_arms[index])[1], ' - ', 'Convergence Day',
                    # d)
        ...
        if len(full_cycle_reward) == exp_params['T']:
    ...
    cl, ncl = np.unique(np.array(noc), return_counts = True)
    curve = np.nancumsum(np.nanmean(opt - ts_final_rewards, axis=0))

    return cl, ncl, curve[-1]
